...
function myalgorithm(alg, inp, ini, final, dir){
        ...
        if(alg=="fcfs"){
                ...
                var target = op[0];
                var seek = op[1];
                var avgSeekTime = op[2];
                var latency = op[3];
                var headMovements = op[4];
                ...
                savePerformanceMetrics("FCFS", seek, avgSeekTime, latency, headMovements);
        }
...
function animation(values) {
        ...
        function speak(text) {
                ...
                msg.rate = 1.2;   // Increase for faster speech (1.0 is normal)
                msg.pitch = 1.0;  // Adjust for higher/lower voice
                msg.lang = "en-US"; // Change to "hi-IN" for Hindi, etc.
        ...
        function animateStep() {
                if (targetIndex >= target.length) {
                    ...
                    speak("FCFS Disk scheduling animation completed.");
                    return;
                }
            
                let from = target[targetIndex - 1];
                let to = target[targetIndex];
            
                // Speak before animation starts
                ...
                setTimeout(() => {
                    ...
                    // Draw all circles
                    circles.forEach(circle => drawCircle(circle, circle.color));
            
                    // Highlight the moving head
                    circles.forEach((circle, i) => {
                        let color = i === to ? "white" : circle.color;
                        ...
                        if (i === to) {
                            ...
                            ctx.fillStyle = "black";
                            ctx.fillText(from, 350 + circles[from].radius, 310);
            
                            // Draw arrow showing movement
                            ctx.beginPath();
                            canvas_arrow(ctx, 350 + circles[from].radius, 320, 350 + circle.radius, 320);
                            ctx.strokeStyle = "red"; // Use red for better visibility
                            ctx.lineWidth = 3;
                            ctx.stroke();
                        }
                    });
                ...
                }, 2000); // Adjusted delay for smooth transition
            }
...
function cscan(inp, ini, final, dir) {
    ...
    function clook(inp, ini, final, dir){
        ...
        if(dir=="left"){
            ...
            seek += Math.abs(hold - inp[inp.length-1]); 
            headMovements++; 
...
async function exportToPDF() {
        ...
        doc.setFontSize(18);
        doc.text("Disk Scheduling Performance Metrics", 10, 10);
        
    
        let sequence = document.getElementById("bitstream-input").value;
        let initialHead = document.getElementById("initial-input").value;
        let finalHead = document.getElementById("final-input").value;
        let algorithm = document.getElementById("algorithm").value;
        
        doc.setFontSize(12);
        doc.text(`Algorithm: ${algorithm}`, 10, 20);
        doc.text(`Sequence: ${sequence}`, 10, 30);
        doc.text(`Initial Head: ${initialHead}`, 10, 40);
        doc.text(`Final Head: ${finalHead}`, 10, 50);
        
        let data = JSON.parse(localStorage.getItem("performanceMetrics")) || [];
        let y = 70;
        
        if (data.length === 0) {
            doc.text("No performance data available.", 10, y);
        } else {
            
            doc.setFontSize(12);
            doc.text("Algorithm", 10, y);
            doc.text("Seek Time", 60, y);
            doc.text("Avg Seek Time", 100, y);
            doc.text("Latency", 140, y);
            doc.text("Head Movements", 180, y);
            y += 10;
            
           
            data.forEach((entry) => {
                doc.text(entry.algorithm, 10, y);
                doc.text(entry.seekTime.toString(), 60, y);
                doc.text(entry.avgSeekTime.toString(), 100, y);
                doc.text(entry.latency.toString(), 140, y);
                doc.text(entry.headMovements.toString(), 180, y);
                y += 10;
            });
        }
        
       
        async function addPlotlyImageToPDF(plotId, posY, newPage = false) {
            let plotDiv = document.getElementById(plotId);
            if (plotDiv) {
                try {
                    let imgData = await Plotly.toImage(plotDiv, { format: 'png', width: 800, height: 500 });
                    if (newPage) doc.addPage();
                    doc.addImage(imgData, 'PNG', 10, posY, 180, 100);
                } catch (error) {
                    console.error(`Error exporting graph ${plotId}:`, error);
                    doc.text(`Error exporting ${plotId} graph`, 10, posY);
                }
            }
        }
    
        
        await addPlotlyImageToPDF("plot", y + 20);
        await addPlotlyImageToPDF("cmpr", 20, true); 
